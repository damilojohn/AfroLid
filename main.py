...
afrolid_model = AutoModel.from_pretrained("damilojohn/afrolid", trust_remote_code=True)


class new_classifier:
    def __init__(self, logger, model, model_path=None):
        self.logger = logger
        self.model_path = model_path
        self.afrolid_task, self.model, self.tokenizer = self.init_task_model(model)
        self.lang_info = self.load_langs_info()
        # print(self.lang_info)

    def init_task_model(self, model):
        # print("dddsssssssss")
    ...
    def classify(self, text, max_outputs=3):
        ...
        # print(tokens)
        tokens = self.afrolid_task.source_dictionary.encode_line(
            tokens,
        ...
        probabilities, predictions_idx = F.softmax(outputs[0], dim=-1).topk(
            k=max_outputs
        )
        # print()
        results = {}
        if max_outputs == 1:
            ...
            predicted_score = round(
                float(torch.squeeze(probabilities).detach().numpy()) * 100, 2
            )
            results[label_name] = {
                "score": predicted_score,
                "name": self.lang_info[label_name]["name"],
                "script": self.lang_info[label_name]["script"],
            }
        else:
            for score, prediction_idx in zip(
                torch.squeeze(probabilities), torch.squeeze(predictions_idx)
            ):
                label_name = self.afrolid_task.target_dictionary.string(
                    [prediction_idx]
                )
                predicted_score = round(
                    float(torch.squeeze(score).detach().numpy()) * 100, 2
                )
...
afrolid = new_classifier(
    logger, model_path="/content/afrolid_model", model=afrolid_model
)
...
predicted_langs = afrolid.classify(text)  # default max_outputs=3
print("Predicted languages:")
for lang in predicted_langs:
    print(
        "     |-- ISO: {}\tName: {}\tScript: {}\tScore: {}%".format(
            lang,
